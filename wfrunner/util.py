...
def get_fastq_pass_dir(dir: str) -> str:
    """Get path of fastq_pass directory from run folder

    Args:
        dir (str): path to runfolder (input path from argparse)

    Returns:
        str: absolute path to fastq_pass folder containing barcoded data
    """
    # get absolute path of basedir
    dir = os.path.abspath(dir)
    # find subdirectory
    for d in os.scandir(dir):
        # check if right subdirectory is found
        if d.name.startswith(os.path.basename(dir)) and d.is_dir():
            # join subdir to basepath
            dir = os.path.join(dir, d)
            # find 'no_sample' directory
            if os.path.exists(os.path.join(dir, 'no_sample')):
                dir = os.path.join(dir, 'no_sample')
                for d in os.scandir(dir):
                    if os.path.exists(os.path.join(dir, d,  'fastq_pass')):
                        dir = os.path.join(dir, d, 'fastq_pass')
    return dir
                    


def create_wd(dir: str) -> str:
    """Create working directory, cd into it

    Args:
        dir (str): working directory

    Returns:
        str: Path to pipeline_execution dir
    """
    dir = os.path.abspath(dir)
    if not os.path.exists(dir):
        os.makedirs(dir)
    workdir = os.path.join(dir, 'pipeline_execution')
    os.makedirs(workdir)
    os.chdir(workdir)
    return workdir
